...
namespace OS_Problem_02
{
    class Thread_safe_buffer
    {
        ...
        static int exitflag = 0; // ตัวแปรสำหรับเก็บค่าเพื่อใช้ในการออกจาก loop ของ thread

        static void EnQueue(int eq) // ฟังก์ชันสำหรับเพิ่มข้อมูลเข้าไปใน buffer
        {
            lock (
                _Lock // ใช้ lock ในการใช้งานของ thread
            )
            {
                while ((Count == TSBuffer.Length) && (exitflag == 0) // ถ้า buffer เต็ม
                )
                {
                ...
                if (
                    exitflag == 0 // ถ้า thread ยังไม่ถึงจุดที่จะออกจาก loop
                )
                {
        ...
        static int
        DeQueue() // ฟังก์ชันสำหรับดึงข้อมูลออกจาก buffer
        {
            ...
            {
                while ((Count == 0) && (exitflag == 0) // ถ้า buffer ว่าง
                )
                {
        ...
        static void th01() // ฟังก์ชันสำหรับ thread ที่ทำงานเพิ่มข้อมูลเข้าไปใน buffer
        {
            for (
                int i = 1;
                i <= 51;
                i++ // ใช้ loop เพื่อเพิ่มข้อมูลเข้าไปใน buffer
            )
            {
                EnQueue (i); // เรียกใช้ฟังก์ชัน EnQueue
                Thread.Sleep(5); // ให้ thread นี้หยุดทำงาน 5 มิลลิวินาที
                if (
                    exitflag == 1 // ถ้า thread ถึงจุดที่จะออกจาก loop
                )
                {
                    break; // ออกจาก loop
                }
        ...
        {
            int i; // ตัวแปรเก็บค่าจำนวนรอบในการวนลูป
            int j; // ตัวแปรเก็บค่าจำนวนข้อมูลที่จะดึงออกจาก buffer

            for (
                i = 0;
                i < 60;
                i++ // ใช้ loop เพื่อดึงข้อมูลออกจาก buffer
            )
            {
        ...
        static void Main(string[] args) // ฟังชั่นหลัก
        {
            ...
            exitflag = 1; // กำหนดให้ thread ที่ทำงานเพิ่มข้อมูลเข้าไปใน buffer ออกจาก loop
            lock (
                _Lock // ใช้ lock เพื่อป้องกันการเข้าถึงข้อมูลใน buffer โดย thread อื่น
            )
            {
